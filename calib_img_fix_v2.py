...
print("=" * 60)
print("LEWA kamera (index 0) = lewa strona robota (patrząc od tyłu)")
print("PRAWA kamera (index 2) = prawa strona robota (patrząc od tyłu)")
print("=" * 60)
...
while capL.isOpened():
    ...
    cv2.putText(imgL_display, "LEWA (index 0)", (10, 30), 
                cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
    cv2.putText(imgR_display, "PRAWA (index 2)", (10, 30), 
    ...
    if k == 27:  # ESC
        ...
        print(f"✓ Para {num} zapisana! (LEWA=index0, PRAWA=index2)")
        num += 1

    cv2.imshow('LEWA Kamera (index 0)', imgL_display)
    cv2.imshow('PRAWA Kamera (index 2)', imgR_display)
