...
def read_f_and_cx(cv_file):
    node = cv_file.getNode('projMatrixL')
    if not node.empty():
        P1 = node.mat()
        return float(P1[0, 0]), float(P1[0, 2]), "projMatrixL"

    node = cv_file.getNode('newCameraMatrixL')
    if not node.empty():
        K = node.mat()
        return float(K[0, 0]), float(K[0, 2]), "newCameraMatrixL"

    node = cv_file.getNode('cameraMatrixL')
    if not node.empty():
        K = node.mat()
        return float(K[0, 0]), float(K[0, 2]), "cameraMatrixL"

    return 500.0, FRAME_WIDTH / 2.0, "default"

FOCAL_LENGTH, CX_L, src = read_f_and_cx(cv_file)
cv_file.release()
...
if TERMINAL_OUTPUT:
    print(f"Using f={FOCAL_LENGTH:.2f}px, cx={CX_L:.2f}px from {src}")
...
def compute_X_center_cm(cx_L, cx_R):
    ...
    Z = (FOCAL_LENGTH * BASELINE) / abs(disp)  # głębia zawsze dodatnia
...
if TERMINAL_OUTPUT:
    print("SYSTEM STARTED (X_center in cm) - press 'q' to quit")
...
while True:
    ...
    if detL is not None:
        ...
        if detR is not None:
            cx_R, cy_R = detR
            X_center, Z, disp = compute_X_center_cm(cx_L, cx_R)
            dist_cm = float(Z) if Z is not None else 0.0
            ...
            if TERMINAL_OUTPUT:
                print(f"DBG cxL={cx_L} cxR=None (no right match)")
    ...
    if GUI_ENABLED:
        ...
        cv2.putText(rectL, f"FPS:{fps:.1f}  f:{FOCAL_LENGTH:.0f}px  cx:{CX_L:.0f}px  ({src})",
                    (10, 25), cv2.FONT_HERSHEY_SIMPLEX, 0.55, (0, 255, 255), 1)

        if X_center is not None and disp is not None:
            cv2.putText(rectL, f"Xc:{X_center:.2f}cm  disp:{disp:.1f}px  Z:{dist_cm:.1f}cm",
